...
/**
 * Procedure for document access: either a signed-in user (owner or invited)
 * or a guest with a valid session for the document.
 */
export const documentAccessProcedure = t.procedure
  .use(timingMiddleware)
  .use(({ ctx, next }) => {
    if (ctx.session?.user) {
      return next({
        ctx: {
          ...
          guestSession: null as { guestId: string; documentId: string } | null,
        },
      });
    }
    if (ctx.guestSession) {
